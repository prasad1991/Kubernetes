...
for i in ret.items:
    ...
    if i.spec.scheduler_name=='PAVA':
       ...
       # print Sorted_poddict[0]
        d = OrderedDict(sorted(group_pod.items(),key=lambda kv:(kv[1],kv[0]),reverse=True))
        list2=[]
        for key,value in d.items():
                list1=list(key.split(" "))
                list2.append(list1)
                for elem in list2:
                        for elem in elem:
                            group_pod1.append(elem)
                             #print (group_pod1)

group_podlist= group_pod1[2:]
print(group_podlist)
...
print(" \n \n sorted PODs namespaces and priority value %s \n \n "%sorted(group_pod.items(),key=lambda kv:(kv[1],kv[0]),reverse=True))
...
print (" \n \n \n MAster and slave compute resources statistics \n \n \n  %s " %data1)
    ...
    if event['object'].status.phase == "Pending"  and event['object'].status.conditions == None and event['object'].spec.scheduler_name == scheduler_name:
        ...
        for item in group_podlist:  
            ...
            try:
                ...
                print (" Pod is succuessfull bind")
